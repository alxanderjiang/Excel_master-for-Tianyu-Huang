...
dataname=input("请输入要计算平均值的数据列序号（C/D/E/F）:")
#打开Excel
...
#计算气温月平均气温
sheet.range(addname+'1').value='月平均'
...
while check < fortimes: 
  ...
  while check<fortimes and sheet.range(str('B'+str(check))).value.month==mon:
    if sheet.range(dataname+str(check)).value!=999999:
       ...
       flag=0
  if flag==1:
    sheet.range(addname+str(number)).value=str(monstartday.year)+'/'+str(monstartday.month)
...
wb.close()
app.quit()
print('***按月平均气象数据计算程序结束 数据保存在'+addname+'列与其后列中***')
print("\n")